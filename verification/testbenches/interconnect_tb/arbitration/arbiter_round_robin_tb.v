`timescale 1ns / 1ps
////////////////////////////////////////////////////////////////////////////////
// Module Name: arbiter_round_robin_tb
// Description: Comprehensive testbench for Write_Arbiter_RR module
//              Tests Round-Robin arbitration between 2 AXI masters
//
// Source File: src/axi_interconnect/rtl/arbitration/algorithms/arbiter_round_robin.v
// Module: Write_Arbiter_RR
// Arbitration Policy: Round-Robin (Fair alternation, no starvation)
//
// Test Cases:
...
module arbiter_round_robin_tb();
    ...
    task test_3_both_request_round_robin;
        begin
            ...
            // First arbitration: Should be M0 (last_served was reset to 1, so M0 selected)
            $display("  Round 1: Expecting M0 (last_served reset to 1, so M0 selected)");
            repeat(2) @(posedge ACLK);  // Wait for registered output
            #1;
            if (Selected_Slave === 0 && Channel_Request === 1) begin
                $display("  [PASS] Round 1: M0 selected correctly");
                pass_count = pass_count + 1;
            end else begin
                $display("  [FAIL] Round 1: Expected M0, got M%0d", Selected_Slave);
                fail_count = fail_count + 1;
            end
            
            // Second arbitration: Should be M1 (M0 was just served, last_served updated)
            $display("  Round 2: Expecting M1 (M0 was last)");
            repeat(2) @(posedge ACLK);  // Wait for registered output
            #1;
            if (Selected_Slave === 1 && Channel_Request === 1) begin
                $display("  [PASS] Round 2: M1 selected correctly");
                pass_count = pass_count + 1;
            end else begin
                $display("  [FAIL] Round 2: Expected M1, got M%0d", Selected_Slave);
                fail_count = fail_count + 1;
            end
            ...
            #1;
            if (Selected_Slave === 0 && Channel_Request === 1) begin
                $display("  [PASS] Round 3: M0 selected correctly");
                pass_count = pass_count + 1;
            end else begin
                $display("  [FAIL] Round 3: Expected M0, got M%0d", Selected_Slave);
                fail_count = fail_count + 1;
            end
    ...
    //==========================================================================
    initial begin
        $dumpfile("arbiter_round_robin_tb.vcd");
        $dumpvars(0, arbiter_round_robin_tb);
    end
