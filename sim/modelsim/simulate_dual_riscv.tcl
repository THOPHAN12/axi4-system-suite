...
if {![file exists [file join $PROJECT_DIR "work"]]} {
    puts "Work library not found. Running compile_all_files.tcl..."
    if {[file exists [file join $PROJECT_DIR "compile_all_files.tcl"]]} {
        source [file join $PROJECT_DIR "compile_all_files.tcl"]
    } else {
        puts "ERROR: compile_all_files.tcl not found!"
        puts "Please compile the design first."
        return
    }
} else {
    puts "Work library exists. Assuming files are compiled."
    puts "If you encounter errors, run: do compile_all_files.tcl"
}
...
# Check if wave config file exists
set WAVE_FILE [file join $PROJECT_DIR "wave_dual_riscv.do"]
