...
//* Write Data Channel
    ...
    input logic  [S00_Write_data_bytes_num-1:0]   S01_AXI_wstrb, // strops identifes the active data lines
...
//*Address Read Channel
    input logic  [Address_width-1:0]     S01_AXI_araddr,// the write address
    input logic  [S01_AR_len-1:0]        S01_AXI_arlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    input logic  [3:0]                   S01_AXI_arcache, // a opptional signal for connecting to diffrent types of  memories
    ...
    input logic                          S01_AXI_arvalid, // Address write valid signal 
    output logic                          S01_AXI_arready, // Address write ready signal 
...
//* Write Data Channel
    ...
    input logic  [S00_Write_data_bytes_num-1:0]   S00_AXI_wstrb, // strops identifes the active data lines
...
//*Address Read Channel
    input logic  [Address_width-1:0]     S00_AXI_araddr,// the write address
    input logic  [S00_AR_len-1:0]        S00_AXI_arlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    input logic  [3:0]                   S00_AXI_arcache, // a opptional signal for connecting to diffrent types of  memories
    ...
    input logic                          S00_AXI_arvalid, // Address write valid signal 
    output logic                          S00_AXI_arready, // Address write ready signal 
...
//* Write Data Channel
    ...
    output logic  [M00_Write_data_bytes_num-1:0]   M00_AXI_wstrb, // strops identifes the active data lines
...
//*Address Read Channel
    output logic  [Address_width-1:0]     M00_AXI_araddr,// the write address
    output logic  [M00_AR_len-1:0]        M00_AXI_arlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    output logic  [3:0]                   M00_AXI_arcache, // a opptional signal for connecting to diffrent types of  memories
    ...
    output logic                          M00_AXI_arvalid, // Address write valid signal 
    input logic                          M00_AXI_arready, // Address write ready signal 
...
//                  /*****************************/
    ...
    output logic  [M00_Write_data_bytes_num-1:0]   M01_AXI_wstrb, // strops identifes the active data lines
    ...
    //*Address Read Channel
    output logic  [Address_width-1:0]     M01_AXI_araddr,// the write address
    output logic  [M01_AR_len-1:0]        M01_AXI_arlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    output logic  [3:0]                   M01_AXI_arcache, // a opptional signal for connecting to diffrent types of  memories
    ...
    output logic                          M01_AXI_arvalid, // Address write valid signal 
    input logic                          M01_AXI_arready, // Address write ready signal 
    ...
    //input logic [Master_ID_Width - 1 : 0]       M1_ID, M2_ID
...
//                  /*****************************/
    ...
    //* Address Write Channel              
    output logic [Slaves_ID_Size-1:0]     M02_AXI_awaddr_ID,
    output logic [Address_width-1:0]      M02_AXI_awaddr,
    output logic [M02_Aw_len-1:0]         M02_AXI_awlen,
    output logic [2:0]                    M02_AXI_awsize,
    output logic [1:0]                    M02_AXI_awburst,
    output logic [1:0]                    M02_AXI_awlock,
    ...
    //* Write Data Channel
    output logic [M02_Write_data_bus_width-1:0]  M02_AXI_wdata,
    output logic [M02_Write_data_bytes_num-1:0]  M02_AXI_wstrb,
    output logic                                  M02_AXI_wlast,
    output logic                                  M02_AXI_wvalid,
    input logic                                  M02_AXI_wready,

    //* Write Response Channel
    input logic [Master_ID_Width-1:0]    M02_AXI_BID,
...
//                  /*****************************/
    ...
    //* Address Write Channel              
    output logic [Slaves_ID_Size-1:0]     M03_AXI_awaddr_ID,
    output logic [Address_width-1:0]      M03_AXI_awaddr,
    output logic [M03_Aw_len-1:0]         M03_AXI_awlen,
    output logic [2:0]                    M03_AXI_awsize,
    output logic [1:0]                    M03_AXI_awburst,
    output logic [1:0]                    M03_AXI_awlock,
    ...
    //* Write Data Channel
    output logic [M03_Write_data_bus_width-1:0]  M03_AXI_wdata,
    output logic [M03_Write_data_bytes_num-1:0]  M03_AXI_wstrb,
    output logic                                  M03_AXI_wlast,
    output logic                                  M03_AXI_wvalid,
    input logic                                  M03_AXI_wready,

    //* Write Response Channel
    input logic [Master_ID_Width-1:0]    M03_AXI_BID,
...
logic Queue_Is_Full;
logic Token;
  logic [(S00_Aw_len/'d2)-1:0] Rem; //Reminder of the divsion
  logic [(S00_Aw_len/'d2)-1:0] Num_Of_Compl_Bursts; // Number of Complete Bursts
...
logic [1:0] wr_turn;  // Write arbitration turn (0=M0, 1=M1)
logic [1:0] rd_turn;  // Read arbitration turn (0=M0, 1=M1)

// Round-robin counter registers (for RR2 - sequential scan)
logic [1:0] wr_counter;  // Write arbitration counter (0=M0, 1=M1, wraps)
logic [1:0] rd_counter;  // Read arbitration counter (0=M0, 1=M1, wraps)

// Master request signals
logic m0_write_req = S00_AXI_awvalid && !AW_Access_Grant;
logic m1_write_req = S01_AXI_awvalid && !AW_Access_Grant;
logic m0_read_req = S00_AXI_arvalid;
logic m1_read_req = S01_AXI_arvalid;

// Grant signals (output of arbitration logic)
logic grant_m0_write;
logic grant_m1_write;
logic grant_m0_read;
logic grant_m1_read;
// ============================================================================
