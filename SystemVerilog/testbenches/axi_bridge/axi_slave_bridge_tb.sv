...
module axi_slave_bridge_tb;
    ...
    task read_single(input [ADDR_WIDTH-1:0] addr, output [DATA_WIDTH-1:0] data);
        @(posedge ACLK);
        s_axi_araddr <= addr;
    ...
    task write_burst_attempt(input [ADDR_WIDTH-1:0] addr, input [7:0] len);
        @(posedge ACLK);
        s_axi_awaddr <= addr;
        ...
        // Wait for response (should be SLVERR)
        wait(s_axi_awready || s_axi_bvalid);
        @(posedge ACLK);
        s_axi_awvalid <= 1'b0;
    ...
    task test_single_write();
        ...
        read_single(test_addr, read_data);
        #(CLK_PERIOD * 2);
        
        check_test("Data written correctly", (read_data == test_data));
        check_test("Read response OKAY", (s_axi_rresp == 2'b00));
    endtask
    ...
    task test_burst_rejection_write();
        ...
        write_burst_attempt(test_addr, 8'h03);
        #(CLK_PERIOD * 5);
        
        // Check that response is SLVERR
        check_test("Burst write rejected with SLVERR", (s_axi_bresp == 2'b10));
    endtask
    ...
    task test_last_signal_enforcement();
        ...
        // Check WLAST is 1 (enforced by bridge)
        check_test("WLAST is 1 for single transaction", (s_axi_wlast == 1'b1));
