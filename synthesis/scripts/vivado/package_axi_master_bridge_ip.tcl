#==============================================================================
# package_axi_master_bridge_ip.tcl
# Package AXI Master Bridge as Vivado IP
...
puts "============================================================================"
puts "Package AXI Master Bridge as Vivado IP"
puts "============================================================================"
puts ""
...
set temp_proj_name "axi_master_bridge_ip_temp"
...
#==============================================================================
# Step 3: Add AXI Master Bridge Source File
#==============================================================================
puts "============================================================================"
puts "Step 3: Adding AXI Master Bridge Source File"
puts "============================================================================"

set bridge_file [file normalize [file join $AXI_BRIDGE_DIR "axi_master_bridge.sv"]]

if {![file exists $bridge_file]} {
    puts "ERROR: AXI Master Bridge file not found: $bridge_file"
    puts "Please create the file first."
    return
}

# Check if file already exists
set existing_files [get_files -quiet [list $bridge_file]]
if {[llength $existing_files] == 0} {
    add_files -norecurse [list $bridge_file]
    set file_obj [get_files -quiet [list $bridge_file]]
    if {[llength $file_obj] > 0} {
        set_property file_type {SystemVerilog} $file_obj
    }
    puts "  Added: [file tail $bridge_file]"
} else {
    puts "  Skipped (already exists): [file tail $bridge_file]"
}

puts ""

# Set top module
set_property top axi_master_bridge [current_fileset]
puts "Top module set: axi_master_bridge"
puts ""
...
set ip_name "axi_master_bridge"
set ip_display_name "AXI Master Bridge"
set ip_description "AXI Bridge for connecting Zynq PS AXI Master to Custom AXI Interconnect. Converts AXI4 GP to AXI4 Full format."
...
puts "     create_bd_cell -type ip -vlnv $ip_vendor:$ip_library:$ip_name:$ip_version axi_master_bridge_0"
puts ""
puts "============================================================================"
