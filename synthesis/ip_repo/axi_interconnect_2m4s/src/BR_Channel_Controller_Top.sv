module BR_Channel_Controller_Top #(
    ...
    input logic [(AXI4_Aw_len/'d2)-1:0] Rem, //Reminder of the divsion
    input logic [(AXI4_Aw_len/'d2)-1:0] Num_Of_Compl_Bursts, // Number of Complete Bursts
...
logic Channel_Request_From_Arb; // Channel_Request output from Write_Resp_Channel_Arb
// wire [Slaves_ID_Size-1:0] Write_Res_Master; // Unused - removed
logic Sele_S_AXI_bready;
// Generate Write_Res_HandShake_En_Pulse from Write_Data_Finsh (edge detector)
logic Write_Data_Finsh_prev;
always_ff @(posedge ACLK or negedge ARESETN) begin
    if (!ARESETN) begin
        Write_Data_Finsh_prev <= 1'b0;
    end else begin
        Write_Data_Finsh_prev <= Write_Data_Finsh;
    end
end
// Write_Res_HandShake_En_Pulse should be asserted when Write Data channel finishes
// AND there's a valid response from a slave (Channel_Request_From_Arb)
assign Write_Res_HandShake_En_Pulse = (Write_Data_Finsh && !Write_Data_Finsh_prev) || Channel_Request_From_Arb; // Rising edge detector OR channel request
...
Write_Resp_Channel_Arb #(
  ...
  u_Write_Resp_Channel_Arb(
      ...
      .Channel_Request (Channel_Request_From_Arb ), //! Output from arbiter
