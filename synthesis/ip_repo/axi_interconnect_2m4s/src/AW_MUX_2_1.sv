...
module AW_MUX_2_1 #(parameter Address_width='d32,S_Aw_len='d8//!AXI4 - 8 bits for burst length
    ...
    input logic  [S_Aw_len-1:0]          S00_AXI_awlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    input logic  [S_Aw_len-1:0]          S01_AXI_awlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    output  logic [S_Aw_len-1:0]          Sel_S_AXI_awlen, // number of transfer per burst //! For AXI4 limit is 256 for inc bursts only 
    ...
    output  logic [3:0]                   Sel_S_AXI_awcache, // a opptional signal for connecting to diffrent types of  memories
